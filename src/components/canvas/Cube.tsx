...
export function Cube({ index, total, character, ...props }: CubeProps) {
  ...
  useFrame((_, delta) => {
    ...
    if (isSelected && (scroll.offset > index / total + 1.35 / total || scroll.offset < index / total - 1.35 / total)) {
      setSelected(null)
    }
  })
    ...
    <mesh
      ...
      onClick={() => {
        if (selected === index) {
          ...
          setSelected(index)
          scroll.el.scrollLeft = ((scroll.el.scrollWidth - scroll.el.clientWidth) / total) * index
        }
      }}
      {...props}>
      <Text ref={nameRef} color='#D4D4D8' fontSize={0.12} position={[0, 0.67, 0]}>
        {character.name}
      </Text>
